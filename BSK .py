...
# ページ設定
st.set_page_config(page_title="バスケ分析Pro V07.2", layout="centered")
...
# ★修正：チーム名を安全に扱うためのState変数（keyとは別にする）
...
with st.sidebar:
    ...
    # === HOME ===
    # ★修正：keyを使わずvalueで状態を渡すことで、上書きエラーを回避
    home_name = st.text_input("自チーム名", value=st.session_state.team_h_name).strip()
    st.session_state.team_h_name = home_name
    ...
    # === AWAY ===
    # ★修正：こちらも同様に安全な渡し方に変更
    away_name = st.text_input("相手チーム名", value=st.session_state.team_a_name).strip()
    st.session_state.team_a_name = away_name
    ...
    with st.expander("📂 過去データを復元・確認 (CSV読込)"):
        st.write("「詳細ログ」のCSVをアップロードしてデータを復元します。")
            ...
            if st.button("データを復元する", type="primary", use_container_width=True):
                try:
                    ...
                    if set(['id', 'Q', 'チーム', '名前', '項目', '詳細', '結果', '点数']).issubset(df.columns):
                        ...
                        # ★修正：チーム名を「key」ではなく独立した「変数」に代入して安全に復元
