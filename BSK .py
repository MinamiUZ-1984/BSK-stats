...
# ページ設定
# ==========================================
st.set_page_config(page_title="松浪ミニバス分析 V68.1", layout="centered")
...
def add_h_player():
    ...
    if new_h:
        ...
        for n in new_nums:
            if n not in all_h_list:
                all_h_list.append(n)
            if n not in curr_act_h:
                curr_act_h.append(n)
...
def add_a_player():
    ...
    if new_a:
        ...
        for n in new_nums:
            if n not in all_a_list:
                all_a_list.append(n)
            if n not in curr_act_a:
                curr_act_a.append(n)
...
def get_team_players(df, team_name, is_home):
    ...
    for d in t_df[t_df['項目'] == 'AST']['詳細'].dropna():
        m = re.search(r'to #(\d+)', str(d).strip())
...
def get_rotation_table(df_hist, p_list, team_name, is_home):
    ...
    for p in p_list:
        ...
        for q in q_cols:
            ...
            # 3. アシストの受け手として記録されているか探す (完全一致に修正)
...
def draw_report_body(df_history, home_name, away_name):
    ...
    def get_full_stats(t_name, p_list_all):
        ...
        for p_num in p_list_all:
            pn = f"{p_num}番"
            pdf = df_history[(df_history['チーム']==t_name) & (df_history['名前']==pn)]
        ...
        for file in sorted(season_files, key=lambda x: x.name):
            try:
                ...
                if 'チーム' in df.columns:
                    # 【超重要】1つのファイルに複数の試合(1Q~4Qが何回も繰り返される)が結合されている場合、
                    # idが1に戻る、もしくは前より小さくなったタイミングで「別の試合」として切り分ける
                    if 'id' in df.columns:
