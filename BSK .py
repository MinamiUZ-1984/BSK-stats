...
# ページ設定
st.set_page_config(page_title="バスケ分析Pro V04.1", layout="centered")

# --- 0. CSS注入 (縦横の余白を極限までカット) ---
st.markdown("""
    <style>
    /* 全体的な余白削減 */
    .block-container { padding-top: 1rem !important; padding-bottom: 0rem !important; }
    
    /* ボタンの横幅を強制的に均等化 & 縦幅をコンパクトに */
    ...
    .stButton > button { 
        ...
        width: 100% !important; 
        font-weight: bold;
        min-height: 42px !important; /* 指で押しやすい最低高さを確保 */
    }
...
def record(item, detail="-", res="成功", pts=0, team=None, name=None):
    ...
    st.session_state.mode = "選手選択"; st.toast("記録完了")

# --- 4. メイン ---
tab_input, tab_report, tab_edit = st.tabs(["✍️ 入力", "📄 レポート", "🛠 修正"])
...
with tab_input:
    ...
    with st.container(border=True):
        if st.session_state.mode == "選手選択": st.info("選手を選んでください")
        elif st.session_state.mode == "項目選択":
            st.write(f"**選手: #{st.session_state.tmp.get('player')}**")
            # 1段目: シュート
            c1 = st.columns(3)
            if c1[0].button("2P", type="primary", use_container_width=True): st.session_state.tmp['item']="2P"; st.session_state.mode="エリア選択"; st.rerun()
            if c1[1].button("3P", type="primary", use_container_width=True): st.session_state.tmp['item']="3P"; st.session_state.mode="エリア選択"; st.rerun()
            if c1[2].button("FT", use_container_width=True): st.session_state.tmp['item']="FT"; st.session_state.mode="結果選択"; st.rerun()
            # 2段目: リバウンド・他
            c2 = st.columns(3)
            if c2[0].button("OR", use_container_width=True): record("OR"); st.rerun()
            if c2[1].button("DR", use_container_width=True): record("DR"); st.rerun()
            if c2[2].button("AST", use_container_width=True): record("AST"); st.rerun()
            # 3段目: 守備・ファウル
            c3 = st.columns(3)
            if c3[0].button("STL", use_container_width=True): record("STL"); st.rerun()
            if c3[1].button("BLK", use_container_width=True): record("BLK"); st.rerun()
            if c3[2].button("F", use_container_width=True): record("Foul"); st.rerun()
            # 4段目: TO
            st.write("▼ TurnOver")
            c4 = st.columns(4)
            for i, val in enumerate(["TV", "DD", "PM", "24S"]):
                if c4[i].button(val, use_container_width=True): record("TO", val); st.rerun()
            ...
            if sc[0].button("SUCCESS", type="primary", use_container_width=True): record(st.session_state.tmp.get('item'), detail=st.session_state.tmp.get('area','-'), res="成功", pts=pts); st.rerun()
...
with tab_report:
    if st.session_state.history.empty: st.info("データなし")
        ...
        st.write(f"🔵 **{home_name}**")
        h_df = get_stats_df(home_name, all_h)
        st.table(h_df.drop(columns='Team').set_index('#'))
        st.write(f"🔴 **{away_name}**")
        a_df = get_stats_df(away_name, all_a)
        st.table(a_df.drop(columns='Team').set_index('#'))
